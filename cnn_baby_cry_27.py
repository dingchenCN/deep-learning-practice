...
#########################
    ...
    def conv2d_160(x, W):
      return tf.nn.conv2d(x, W, strides=[1, 160, 1, 1], padding='VALID') #输入输出一样维度  [batch, height, width, channels].
    ...
    def conv2d_4(x, W):
      return tf.nn.conv2d(x, W, strides=[1, 4, 1, 1], padding='VALID') #输入输出一样维度  [batch, height, width, channels].

    def max_pool_2x1(x):
      return tf.nn.max_pool(x, ksize=[1, 2, 1, 1],        # [batch, height, width, channels]   ksize: A list of ints that has length >= 4. The size of the window for each dimension of the input tensor.
...
#########################
    ...
    # 实际卷积处
    h_conv1 = tf.nn.relu(conv2d_160(x_sound, W_conv1) + b_conv1) # 696*1
    h_pool1 = max_pool_3x1(h_conv1) # 232*1

    W_conv2 = weight_variable([4, 1, 32, 64])
    b_conv2 = bias_variable([64])

    h_conv2 = tf.nn.relu(conv2d_4(h_pool1, W_conv2) + b_conv2)  # 58*1
    h_pool2 = max_pool_2x1(h_conv2) # 29*1

    W_fc1 = weight_variable([29 * 1 * 64, 1024]) # 1024 自设？？？
    b_fc1 = bias_variable([1024])

    h_pool2_flat = tf.reshape(h_pool2, [-1, 29*1*64])
